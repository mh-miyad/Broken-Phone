...
// const loadPhones = async (searchText,data1) => {
//   const res = await fetch(
//     `https://openapi.programming-hero.com/api/phones?search=${searchText}`
//   );
//   const data = await res.json();
//   toggleSpinner(false);
//   displayPhone(data.data ,data1);
// };

// const displayPhone = (allPhones,data1) => {
//   const phonesContainer = document.getElementById("phones-container");
//   phonesContainer.textContent = "";
//   const showAll = document.getElementById("show-all");
//   if (data1 && allPhones.length > 10) {
...
const displayPhone = (allPhones, data1) => {
  ...
  const showAll = document.getElementById("show-all");
  if (data1 && allPhones.length > 10) {
    allPhones = allPhones.slice(0, 10);
    showAll.classList.remove("d-none");
  } else {
    showAll.classList.add("d-none");
  }
...
showAllBtn.addEventListener("click", () => {
  loadPhones(valueSarch.value, true);
});
...
// Load the phones with the "apple" search query and show only the first 10 results
loadPhones("apple")
